...
// LOCALSTORAGE
// ==========================
function simpanData(markahBaru, jawapanRekod) {
    const highScore = parseInt(localStorage.getItem("highScore") || "0");
    if (markahBaru > highScore) {
        localStorage.setItem("highScore", markahBaru);
    }
    const percubaan = parseInt(localStorage.getItem("percubaan") || "0");
    localStorage.setItem("percubaan", percubaan + 1);

    // Kira bilangan soalan yang benar-benar dijawab (betul atau salah, bukan skip)
    const dijawabCount = jawapanRekod.filter(j => j === "betul" || j === "salah").length;

    // Simpan jumlah soalan dijawab secara kumulatif
    const dijawabLama = parseInt(localStorage.getItem("totalDijawab") || "0");
    localStorage.setItem("totalDijawab", dijawabLama + dijawabCount);

    const tarikhSekarang = new Date().toLocaleString("ms-MY");
    localStorage.setItem("rekodTerakhir", JSON.stringify({
        tarikh:   tarikhSekarang,
        markah:   markahBaru,
        jumlah:   soalanKuiz.length,
        dijawab:  dijawabCount,
        jawapan:  jawapanRekod
    }));
    localStorage.removeItem("kuizProgress");
}

function simpanProgress() {
    localStorage.setItem("kuizProgress", JSON.stringify({
        nomborSoalan,
        markah,
        rekodJawapan
    }));
...
function muatProgress() {
    const data = localStorage.getItem("kuizProgress");
    if (!data) return false;
    const p = JSON.parse(data);
    nomborSoalan = p.nomborSoalan;
    markah       = p.markah;
    rekodJawapan = p.rekodJawapan;
    return true;
}

function kemaskinHomepage() {
    const highScore = localStorage.getItem("highScore") || "0";
    const percubaan = localStorage.getItem("percubaan") || "0";
    document.getElementById("high-score").textContent      = highScore + "%";
    document.getElementById("jumlah-percubaan").textContent = percubaan + "x";

    const rekodRaw = localStorage.getItem("rekodTerakhir");
    const btnTerus = document.getElementById("button-teruskan");
    const rekodBox = document.getElementById("rekod-terakhir");

    if (rekodRaw) {
        const rekod = JSON.parse(rekodRaw);

        // Paparan "Soalan Dijawab" — gunakan nilai dari rekod terakhir
        // Sokong format lama (markah) dan format baru (dijawab)
        const dijawabVal = (rekod.dijawab !== undefined) ? rekod.dijawab : rekod.markah;
        document.getElementById("jumlah-jawab").textContent = dijawabVal + " / " + rekod.jumlah;

        rekodBox.style.display = "block";
        document.getElementById("rekod-tarikh").textContent = rekod.tarikh;

        const grid = document.getElementById("rekod-jawapan");
        grid.innerHTML = "";
        rekod.jawapan.forEach((j, i) => {
            const div = document.createElement("div");
            div.className = "rekod-item " + (j === "betul" ? "rekod-betul" : j === "salah" ? "rekod-salah" : "rekod-skip");
            div.title = "Soalan " + (i + 1) + ": " + j;
            div.textContent = i + 1;
            grid.appendChild(div);
        });
    } else {
        rekodBox.style.display = "none";
        document.getElementById("jumlah-jawab").textContent = "0 / 15";
    }

    btnTerus.style.display = localStorage.getItem("kuizProgress") ? "flex" : "none";
}
...
function ulangSoalan() {
    ...
    rekodJawapan[nomborSoalan] = "skip";
    sudahJawab = false;
    paparSoalan();
}
...
function paparResult() {
    ...
    simpanData(peratus, rekodJawapan);
...
function resetSemuaData() {
    playSound(bunyiClick);
    if (!confirm("Adakah anda pasti ingin memadam semua data? Markah, rekod, dan pilihan watak akan dipadamkan.")) return;
    localStorage.clear();
    genderDipilih = "girl";
    nomborSoalan  = 0;
    markah        = 0;
